...
const FileUpload = () => {
    ...
    return (
        <div className='w-full flex gap-2 flex-col justify-around align-middle items-center'>
            ...
            {uploadedFile && typeof uploadedFile === 'string' && (
                <div className={`text-center relative w-[70%] ${loading ? 'loading' : ''}`}>
                    <img src={uploadedFile} alt="Uploaded" className={`w-[100%] h-[50vh]`} />
                </div>
            )}
            {
                resultURL && <div className='relative w-[70%]'>
                    <img src={resultURL} alt="Uploaded" className="w-[100%] h-[50vh]" />
                </div>
            }
            {(resultURL || uploadedFile) && <div className='w-[40%] px-4 py-2 flex flex-col gap-5 text-center'>
                {
                    errors.message && <div className='p-3 rounded tracking-wide bg-black text-sm text-white'>{errors.message}</div>
                }
                {
                    resultURL && <a href={resultURL} target="_blank" download className='px-4 py-2 text-center rounded w-[100%] bg-slate-950 text-white'>DOWNLOAD</a>
                }
                {
                ...
                {
                    (errors.message || resultURL) && <button onClick={() => { setUploadedFile(null), setLoading(false), setErrors({}) }} className='px-4 py-2 text-center rounded w-[100%] border'>Upload Again</button>
                }
            </div>}
        </div>
    );
